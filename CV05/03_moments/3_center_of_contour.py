...
print(f"\nSt3: flag={flag}: {ttle} image moments's center:\ncenter_img=", center_img)    # 영상 전체의 중심점
...
cv2.circle(img2, center_img, 10, (0, 0, 255), -1)
cv2.circle(img2, center_img, 8, (0, 255, 255), -1)
cv2.circle(img2, center_img, 4, (0, 0, 255), -1)
...
# 검출된 객체의 총 수를 출력한다.
print('\nSt4: Number of total contours = ', len(contours) )
...
xx = int(avg_centr[0]+0.5)
yy = int(avg_centr[1]+0.5)
avg_centr = (xx, yy)
print("average of contour mmoments's centers:\navg_centr=", avg_centr)

# 각 오브젝트의 평균 센터를 컬러 동심원(노란 바탕에 빨간색)으로 표기해 보자.
cv2.circle(img2, avg_centr, 10, (0, 0, 255), -1)
cv2.circle(img2, avg_centr, 8, (0, 255, 255), -1)
cv2.circle(img2, avg_centr, 4, (0, 0, 255), -1)
cv2.imshow("St4: Avg. of contour moments's centers", img2)
cv2.waitKey()

#print("contour moments's centers=\n", avg_centr)
#print(f"image moment center using {ttle} image  ={center_img}")
#cv2.imshow("St4: contours moments's centers on gray", img2)
