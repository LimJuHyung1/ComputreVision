...
cv2.ml_KNearest.train(samples, layout, responses) -> retval
 ...
 참조 링크
    OpenCV로 k 최근접 이웃 알고리즘(KNN) 구현하기
    https://deep-learning-study.tistory.com/286
...
# k-NN training:
knn.train(data, cv2.ml.ROW_SAMPLE, labels)  # 하나의 데이터가 한 행으로 구성됨
# k-NN find nearest:
k = 3
...
# Set the title:
if results[0][0] > 0:
    plt.suptitle(f"k-NN: green point classified as blue (k={k})", fontsize=14,
                 fontweight='bold')
else:
    plt.suptitle(f"k-NN: green point classified as red (k={k})", fontsize=14,
                 fontweight='bold')
...
# 반지름=제일 먼 데이터의 거리. k-1번째.거리가 L2-norm이라 root처리가 필요.
...
shp=patches.Circle((sample[0, 0], sample[0, 1]), radius=dist_max,
