...
def load_digits_and_labels(big_image):
    ...
    return digits, labels   # (5000,)개의 영상과 레이블을 반환한다.
...
#   >>> np.random.permutation(10) # 0~9까지의 수자를 뒤섞는다.
#   array([1, 7, 4, 3, 0, 9, 2, 5, 8, 6]) # random
#   >>> np.random.permutation([1, 4, 9, 12, 15])
#   array([15,  1,  9,  4, 12]) # random
# ------------------------------------------------------------------------------------------------------
print('\n@@2. Training data will be shuffled.')
...
#raw_descriptors = np.squeeze(raw_descriptors)  # 한 축이 1의 원소를 갖는 자료가 없어 squeeze할 것이 없지만 리스트 자료를 ndarray로 만들 수 있다..
...
# ------------------------------------------------------------------------------------------------------
# 4. 주어진 5,000개의 학습용 데이터를 knn 모델링하기 위한 학습 데이터와 테스트 데이터로 반씩 나눈다.
# ------------------------------------------------------------------------------------------------------
...
# indices_or_sections가 [a, b, c]인 경우 첫번재 축의 a에 나누고, b에서 나누고. c에서 나눈다. [a]이면 a에서 나눈다.
# axis는 나누는데 기준이 되는 축으로 첫번째=0, 두번째=1, ..

# raw_descriptors와 labels를 index 번호(=partition)로 나누고자 한다. -> 2개의 정보를 모두 2개로 나누어야 한다.
...
# 위는 [인덱스] 기법으로 나눈 것이고 아래처럼 간단히 2개로 나눈어도 된다.
...
# 6. 나머지 반의 데이터로 테스트를 행하고 분류 성능을 분석한다.
#   사용되는 함수: 결과=findNearest(테스트_데이터, k)
#   k변화에 따른 정확도 변화를 관찰한다. 학습된 데이터를 테스팅하니 K=1이면 100% 정확도를 보였다.
#
...
#   dist: 주변의 k개가 현 test 데이터와 얼마만큼의 떨어져 있는지 보인다. 거리는 L2-norm이다.
