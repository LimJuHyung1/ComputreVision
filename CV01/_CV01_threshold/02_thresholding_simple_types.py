...
threshold(src, 50, 255, CV_THRESH_BINARY);
    50 넘는 src 화소에 대해서는 255를 반환한다. 아니면 0을 반환.

threshold(src, 50, 255, CV_THRESH_BINARY_INV);
    50 넘는 src 화소에 대해서는 0 을 반환한다. 아니면 255를 반환.

threshold(src, 100, 255, CV_THRESH_TRUNC);
    100을 넘는 src 화소에 대해서는 100을 반환. 이하의 값은 원본 값을 그대로 반환한다. 255는 활용되지 않는다.

threshold(src, 100, 255, CV_THRESH_TOZERO);
    100을 넘으면  원본 값을 반환. 그 이하의 값은 0을 반환한다. 255는 활용되지 않는다.
